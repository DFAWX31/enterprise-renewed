...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('monthly')
		.setDescription('To claim every month'),
	async execute(interaction: CommandInteraction) {
		...
		if (parseInt(getEntry.monthly) + 2592000 >= parseInt(date)) {
			return await interaction.reply({
				content: "You can't do that yet as it's not been a month yet",
				ephemeral: true
			})
		}
		...
		const money = getUser?.balance + 1500
		...
		await interaction.reply("You have earned 1500 coins")
	}
}